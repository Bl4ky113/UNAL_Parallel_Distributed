...
public final class ReciprocalArraySum {
    ...
    protected static double parArraySum(final double[] input) {
        ...
        double sum = 0;
        int indexSplit = input.length / 2;
        
        ReciprocalArraySumTask leftSumTask = new ReciprocalArraySumTask(0, indexSplit, input);
        ReciprocalArraySumTask rightSumTask = new ReciprocalArraySumTask(indexSplit, input.length, input);
        
        leftSumTask.fork();
        rightSumTask.compute();

        leftSumTask.join();

        sum = leftSumTask.getValue() + rightSumTask.getValue();
        return sum;
    }
